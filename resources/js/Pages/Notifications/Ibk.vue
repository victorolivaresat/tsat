  ...
  <PermissionCheck :permission="'users.index'">
      ...
      <div class="bg-white rounded-md shadow overflow-x-auto">
        <table class="w-full whitespace-nowrap">
          ...
          <tr v-for="transaction in ibk.data" :key="transaction.id"
            class="hover:bg-gray-100 focus-within:bg-gray-100">
            <td class="border-t px-6 py-3 text-indigo-600"><GlDotGrid/></td>
            <td class="border-t px-6 py-3 text-green-400">{{ transaction.beneficiary }}</td>
            <td class="border-t px-6 py-3">{{ transaction.number_application }}</td>
            ...
            <td class="w-px border-t">
              <Link class="flex items-center px-4" :href="`/ibk/${transaction.id}/edit`" tabindex="-1">
              <icon name="cheveron-right" class="block w-6 h-6 fill-gray-400" />
              </Link>
            </td>
          </tr>
          <tr v-if="ibk.data.length === 0">
            <td class="px-6 py-4 border-t" colspan="8">No transactions found.</td>
          </tr>
        </table>
      </div>
      <pagination class="mt-6" :links="ibk.links" />
  ...
  watch: {
    form: {
      ...
      handler: throttle(function () {
        this.$inertia.get('/transactions/ibk', pickBy(this.form), { preserveState: true })
      }, 150),
    },
  },
  methods: {
